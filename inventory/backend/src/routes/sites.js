...
import { newId, isUniqueConstraintError, parseSiteCsvRow, normalizeSiteRowForInsert } from '../utils/helpers.js';
import { processCombinedImport } from '../utils/combinedImport.js';
import { parseUploadCsvBuffer } from '../utils/csvUpload.js';
import { csvUploadFileFilter, createRateLimiters, getSecurityConfig, escapeCsvCell } from '../middleware/security.js';
...
router.post('/', (req, res) => {
  ...
  if (!name || !plaid || !territoryVal || !region) {
  ...
  try {
    db.prepare(
      `INSERT INTO sites (id, name, plaid, area, territory, region, address, lat, lng)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(id, name, plaid, territoryVal, territoryVal, region, address ?? null, latN, lngN);
  } catch (e) {
