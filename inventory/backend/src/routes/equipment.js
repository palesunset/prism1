...
import { csvUploadFileFilter, createRateLimiters, getSecurityConfig } from '../middleware/security.js';
...
router.post('/', (req, res) => {
  ...
  const ip =
    ip_address === undefined || ip_address === null || String(ip_address).trim() === ''
      ? null
      : String(ip_address).trim();
  const rt =
...
router.post('/import', uploadLimiter, upload.single('file'), async (req, res) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const result = importEquipmentFromParsed(siteId, parsed, existingSerials, batchSerials);
    if (result.ok) {
      added++;
    } else {
...
router.patch('/:id', (req, res) => {
  ...
  if (ip_address !== undefined) {
    ...
    vals.push(
      ip_address === null || ip_address === '' || String(ip_address).trim() === ''
        ? null
        : String(ip_address).trim()
    );
  }
