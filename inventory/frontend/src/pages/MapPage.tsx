...
import { useSitesList, useSiteMutations } from '@/hooks/useSites';
...
function siteTerritoryValue(s: Site): string {
...
export function MapPage() {
  ...
  const regions = useMemo(() => {
    const subset = sites.filter(
      (s) =>
        (mapTerritoryFilter.length === 0 || mapTerritoryFilter.includes(siteTerritoryValue(s))) &&
        siteMatchesRouterTypesMulti(s, mapRouterTypeFilter)
    );
    return uniqueSortedStrings(subset.map((s) => s.region));
  }, [sites, mapTerritoryFilter, mapRouterTypeFilter]);
  ...
  const areas = useMemo(() => {
    ...
    return uniqueSortedStrings(subset.map((s) => siteTerritoryValue(s)));
  }, [sites, mapRegionFilter, mapRouterTypeFilter]);
    ...
    () =>
      sites.filter(
        (s) =>
          (mapRegionFilter.length === 0 || mapRegionFilter.includes(s.region)) &&
          (mapTerritoryFilter.length === 0 || mapTerritoryFilter.includes(siteTerritoryValue(s))) &&
          (mapRouterTypeFilter.length === 0 ||
            mapRouterTypeFilter.some((t) =>
              String(s.equipment_router_types || '')
                .split(',')
                .map((x) => x.trim())
                .filter(Boolean)
                .includes(t)
