...
CREATE TABLE IF NOT EXISTS ip_workflows (
  ...
  state TEXT NOT NULL DEFAULT 'REQUESTED' CHECK(state IN (
    'REQUESTED', 'VALIDATED', 'PENDING_APPROVAL', 'APPROVED',
    'RESERVED', 'ACTIVE', 'MODIFIED', 'DECOMMISSIONED'
  )),
  netlens_result TEXT,
  ipam_record_id TEXT,
...
try {
  db.exec(`
CREATE UNIQUE INDEX IF NOT EXISTS idx_ip_records_unique_host ON ip_records(range_start) WHERE record_type = 'host';
`);
} catch {
  /* index may fail if duplicates already exist — integrity scan will flag them */
}
