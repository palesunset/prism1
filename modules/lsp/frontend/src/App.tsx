...
export default function App() {
  ...
  const reloadTopology = useCallback(async () => {
    ...
    const nextSource = st.source && inGraph.has(st.source) ? st.source : "";
    let nextDest = st.destination && inGraph.has(st.destination) ? st.destination : "";
    if (nextSource && nextDest && nextSource === nextDest) {
      nextDest = "";
    }
  ...
  return (
    <div
      className="lsp-shell flex h-full min-h-0 flex-col"
      onDragOver={(e) => {
        e.preventDefault();
        setDragHint(true);
      }}
