...
import { parseIpForStorage, findDuplicateIpEquipment } from '../utils/ipAddress.js';
...
export function validateEquipmentRow(parsed) {
  ...
  if (parsed.ip_address) {
    const ipParsed = parseIpForStorage(parsed.ip_address);
    if (!ipParsed.ok) return ipParsed.error;
    parsed.ip_address = ipParsed.value;
  }
...
export function importEquipmentFromParsed(siteId, parsed, existingSerials, batchSerials, batchIps = new Set()) {
  ...
  if (parsed.ip_address) {
    ...
    if (dupes.length > 0) {
      return {
        ok: false,
        error: `IP ${parsed.ip_address} is already assigned to ${dupes[0].serial_number}`,
      };
    }
  }
