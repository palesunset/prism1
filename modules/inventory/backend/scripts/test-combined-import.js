...
function runIntegrationTests() {
  ...
  // 9. International site — Region ITL + Territory number (Singapore-style rows)
  const r9 = processCombinedImport([
    {
      'Site Name': 'SINGAPORE EQUINIX SG1',
      PLAID: 'TEST-COMB-ITL-A',
      Region: 'ITL',
      Territory: '7',
      Address: 'Blk 20, Ayer Rajah Crescent Level 4 #05-05/08 Singapore 139964',
      Latitude: '1.295411473',
      Longitude: '103.7897736',
    },
  ]);
  eq(r9.sites_added, 1, 'international site with Region ITL');
  eq(r9.errors.length, 0, 'no errors on ITL region row');
  const intlSite = db.prepare('SELECT region, area FROM sites WHERE plaid = ?').get('TEST-COMB-ITL-A');
  eq(intlSite.region, 'INTERNATIONAL', 'ITL maps to INTERNATIONAL region');
  eq(intlSite.area, 'ITL', 'international territory stored as ITL');
  db.prepare('DELETE FROM sites WHERE plaid = ?').run('TEST-COMB-ITL-A');

  // 10. International site — Territory ITL + Region numeric (swapped columns)
  const r10 = processCombinedImport([
    {
      'Site Name': 'SINGAPORE EQUINIX SG1',
      PLAID: 'TEST-COMB-ITL-B',
      Region: '7',
      Territory: 'ITL',
    },
  ]);
  eq(r10.sites_added, 1, 'international site with Territory ITL');
  eq(r10.errors.length, 0, 'no errors on swapped ITL columns');
  const intlSite2 = db.prepare('SELECT region, area FROM sites WHERE plaid = ?').get('TEST-COMB-ITL-B');
  eq(intlSite2.region, 'INTERNATIONAL', 'numeric region with ITL territory maps to INTERNATIONAL');
  eq(intlSite2.area, 'ITL', 'ITL territory preserved');
  db.prepare('DELETE FROM sites WHERE plaid = ?').run('TEST-COMB-ITL-B');
