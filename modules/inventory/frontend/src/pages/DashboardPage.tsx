...
import { useDashboardData, useEquipmentUtilization, useInvalidateDashboard } from '@/hooks/useDashboardData';
...
export function DashboardPage() {
  ...
  const [equipmentSiteId, setEquipmentSiteId] = useState('');
  const [equipmentUtilSort, setEquipmentUtilSort] = useState('util_desc');
  ...
  const {
    data: equipmentUtilData,
    isLoading: equipmentUtilLoading,
    isFetching: equipmentUtilFetching,
  } = useEquipmentUtilization(apiFilters, equipmentSiteId || null, equipmentUtilSort);

  const siteOptionsForEquipment = useMemo(() => {
    if (!data?.siteUtilization.sites.length) return [];
    return [...data.siteUtilization.sites].sort((a, b) => a.name.localeCompare(b.name));
  }, [data?.siteUtilization.sites]);
  ...
  useEffect(() => {
    if (!equipmentSiteId) return;
    const allowed = new Set(siteOptionsForEquipment.map((s) => s.id));
    if (!allowed.has(equipmentSiteId)) {
      setEquipmentSiteId('');
    }
  }, [equipmentSiteId, siteOptionsForEquipment]);
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            onClick={() => {
              void downloadGlobalExportCsv().catch((err: unknown) => {
                const msg =
      ...
      {isLoading && !data ? (
        ...
        <>
          ...
          <CollapsibleChartCard
            title="Port utilization by equipment"
            actions={
              <div className="flex flex-wrap items-center gap-2">
                <select
                  value={equipmentSiteId}
                  onChange={(e) => setEquipmentSiteId(e.target.value)}
                  className="max-w-[min(100%,14rem)] rounded-lg border border-slate-300 bg-white px-2 py-1 text-xs dark:border-slate-600 dark:bg-slate-900"
                  aria-label="Filter by site"
                >
                  <option value="">Select site…</option>
                  {siteOptionsForEquipment.map((s) => (
                ...
                <select
                  value={equipmentUtilSort}
                  onChange={(e) => setEquipmentUtilSort(e.target.value)}
                  disabled={!equipmentSiteId}
                  className="rounded-lg border border-slate-300 bg-white px-2 py-1 text-xs disabled:opacity-50 dark:border-slate-600 dark:bg-slate-900"
                >
                  <option value="util_desc">Utilization (high → low)</option>
                  <option value="util_asc">Utilization (low → high)</option>
                  <option value="name">Equipment (A–Z)</option>
                </select>
              </div>
            }
          >
            {!equipmentSiteId ? (
              <p className="py-8 text-center text-sm text-slate-500">
                Select a site above to view port utilization per equipment.
              </p>
            ) : equipmentUtilLoading && !equipmentUtilData ? (
              <SkeletonChart />
            ) : (
              <>
                {equipmentUtilData?.siteName ? (
                  <p className="mb-3 text-sm font-medium text-slate-700 dark:text-slate-200">
                    {equipmentUtilData.siteName}
                    {equipmentUtilFetching ? (
                ...
                <EquipmentUtilizationChart equipment={equipmentUtilData?.equipment ?? []} />
              </>
            )}
          </CollapsibleChartCard>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <CollapsibleChartCard title="Equipment by territory / region">
              <AreaBreakdownChart byArea={data.areaRegion.byArea} byRegion={data.areaRegion.byRegion} />
            </CollapsibleChartCard>
