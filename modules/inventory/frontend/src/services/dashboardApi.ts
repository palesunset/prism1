...
import type { DashboardBundle, DashboardFilters, EquipmentUtilizationResponse } from '@/types/dashboard';
...
export async function fetchEquipmentUtilization(
  f: DashboardFilters,
  siteId: string,
  sort = 'util_desc'
): Promise<EquipmentUtilizationResponse> {
  const params = toParams(f, { site_id: siteId, sort });
  const { data } = await client.get<EquipmentUtilizationResponse>('/dashboard/equipment-utilization', {
    params,
  });
  return data;
}
