# LSP Simulator User Manual

## Purpose

The LSP Simulator helps you reason about RSVP-TE / SR-MPLS / SRv6 style tunnel designs on a physical graph:
...
| `role` | No | `core`, `agg`, `edge` (default `agg`) — affects default node styling. |
...
The PyInstaller bundle runs a local HTTP server and opens a browser tab. No cloud services are required. Keep in mind that “single file” (`--onefile`) packaging trades startup time for convenience; this repository defaults to **onedir** style output for faster cold start.
