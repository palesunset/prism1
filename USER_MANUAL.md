...
1. **Inventory** — equipment and sites  
2. **LSP Design** — path computation and simulation  
...
- Default model path: `inventory/backend/models/llama-3.2-3b-instruct-q4_k_m.gguf` (~2 GB)
- Configure a different file in `inventory/backend/.env`: `OZ_MODEL_PATH=...`
...
- Data stored locally in `notes/backend/notes.db`
...
Network engineering diagnostic tool — runs entirely in the browser (no backend).

- Enter CIDR (`192.168.1.10/27`), combined IP+mask, or split fields
- **Overview** — network, broadcast, mask, block size
...
- **Classification** — RFC1918, public, loopback, link-local, CGNAT
...
Variable-length subnet planner from the PRISM menu.

1. Enter a **base network** (e.g. `10.0.0.0/24`).
2. Add rows for each site with **required host counts**.
...
2. Enter an IP (`192.168.1.10`), CIDR (`192.168.1.0/27`), base network, or VLSM-style host list (`hosts: 50, 20, 10`).
...
```
NetLens (validate)  →  IP Workflow (approve)  →  Registry (persist)
```
...
Full local IP address management at home tile #3 or `/ipam`.

### Tabs

| Tab | Purpose |
| --- | --- |
| **Dashboard** | Per-subnet utilization cards and summary KPIs |
| **Registry** | Source-of-truth CRUD; expandable subnet tree with nested hosts |
| **Subnets** | Subnet list with project/location/VLAN preview; full subnet details, free ranges, next-IP suggestion, host allocation with per-host metadata |
...
| **Analytics** | Project breakdown, utilization table, downloadable report |
...
- **Next IP** — fills and assigns the next free address in the subnet
- **Manual entry** — type any address within the subnet and click **Allocate**

If the address is already registered, the app shows an inline error (for example *“10.0.0.5 is already in use (Router)”*) instead of crashing. The same duplicate protection applies when adding records from **Registry**.
...
| **APPROVED** | Approved; registry entry created as **reserved** |
| **RESERVED** | Held in registry as reserved |
| **ACTIVE** | In production use (**used** in registry) |
...
| **Reject** | Returns request to `REQUESTED` |
...
- Registry is written on **approve** (as reserved), **reserve**, and **activate** (as used) — not when the request is first created.
- **Re-run NetLens** refreshes validation on `REQUESTED` or `VALIDATED` items.

Direct **Registry** and **System Control** imports remain available for bulk or admin work outside the workflow pipeline.
...
Data is stored in `ipam/backend/ipam.db` via the IPAM API (port **3003**).
...
This exercises registry CRUD, search, VLSM import, duplicate-host rejection (HTTP 409), subnet status updates with hosts inside, integrity audit endpoints, and the **IP Workflow** lifecycle (create → validate → approve → activate → decommission). Requires the IPAM API on port **3003** (started automatically with `npm run dev`).
...
1. Copy `inventory/backend/.env.example` to `inventory/backend/.env`.
2. Generate a key:
